...
* as: space + and + space ( and )
* saps: space + ampersand + space ( & )
* sbsaps: space + backslash + ampersand + space ( \\& ) (for LaTeX)
* sas: space + and + space ( and )
* n: none ()
...
1. Author block
   ...
   Author names' expression is controlled by the parameters of `AuthorStyle`, `LastDelimiterIgnore`, `ManyAuthors`, `ManyAuthorsOption`, and `EtAlExpression`.
   ...
   * If `LastDelimiterIgnore = yes`, the delimiter (= *x2* in `AuthorStyle`) before the last author's name will be ignored. 
